...
【重要】ブランド名は必ず「{brand}」をそのまま使用してください（他のブランドに変更禁止）。
                ...
                try:
                    ...
                    try:
                        ...
                        if ai_cfg.get("provider", "none") != "none":
                            ...
                            src_title = f"{brand} {name_kr}".strip()  # 브랜드 + 원본 상품명
                            title_prompt = f"""BUYMAの出品タイトルを60文字以内で作成してください。
タイトルのみ出力してください（説明不要）。

【重要ルール】
- ブランド名は必ず「{brand}」をそのまま使用（他のブランドに変更禁止）
- 韓国限定・日本未入荷のアピールを含める
- 検索されやすいキーワードを入れる
